...
在上图中，每一个圆圈的概率\\(v_j(t) = max_{q_0,\dots,q_t}P(q_0,\dots,q_{t-1}, o_0,\dots, o_t,q_t = j | \lambda)\\)，通过前面每一步的最可能的路径找到最优的隐变量状态序列，即每一个状态的概率由如下计算：
\\[ v_j(t) = max_{q_1, \dots, q_{t-1}}P(q_0, \dots, q_t, o_0, \dots, o_t, q_t = j | \lambda)
\\]
