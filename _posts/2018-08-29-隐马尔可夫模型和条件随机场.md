...
<pre><code>
func Forward(observations of len T, state-graph of len N)
    create a probability matrix forward[N+2, T]
    for each state s from 1 to N
         \(forward[1,s] = a_{0,s}b_s(o_1)\)
    for each step t from 2 to T
         for each state s from 1 to N
         	\( forward[t, s] =  \sum_{s^'}forward[t-1,s^']a_{s^',s}b_{s}(o_t)\)
</code></pre>
